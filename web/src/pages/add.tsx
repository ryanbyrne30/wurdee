...
function QuoteForm({ setError }: {setError: Dispatch<SetStateAction<string | null>>}) {
...
function WordForm({ setError }: {setError: Dispatch<SetStateAction<string | null>>}) {
  ...
  const createMutation = trpc.word.create.useMutation();
  ...
  const onSubmit = (e: FormEvent) => {
    ...
    if (!parsed.success) return setError(`Invalid input. ${parsed.error.toString()}`);
